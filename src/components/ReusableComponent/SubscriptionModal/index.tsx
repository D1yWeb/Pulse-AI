"use client";
import { onGetStripeClientSecret, activateUserSubscription } from "@/action/stripe";
  ...
  const handleConfirm = async () => {
    try { 
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        const result = await activateUserSubscription(user.id);
        if (result.success) {
          toast.success("Subscription activated successfully!");
        } else {
          console.error("Failed to activate subscription:", result.error);
          // Still show success since payment went through, webhook will update it
          toast.success("Payment successful! Subscription will be activated shortly.");
        }
      }
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Pulse AI Subscription</DialogTitle>
