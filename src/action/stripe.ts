...
// Update subscription status directly after payment
export const activateUserSubscription = async (userId: string) => {
  try {
    await prismaClient.user.update({
      where: { id: userId },
      data: {
    ...
    return { status: 500, success: false, error: "Failed to activate subscription" };
  }
};
